...
- **Self-update**: Always checks for updates and displays them on startup. Performs automatic pull and restart only when `enable_auto_update = true`. If `enable_auto_update = false` (default, including unset), it only detects and displays updates without applying them. If true, it continues checking for updates every minute even during the monitoring loop. Verbose auto-update debug logs are shown only when `enable_auto_update_debug_log = true`.
