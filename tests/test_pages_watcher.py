...
class TestGetPagesLatestBuild:
    ...
    def test_returns_build_on_success(self):
        mock_result = MagicMock()
...
class TestCheckPagesDeployment:
    ...
    def test_deployed_when_built_and_sha_matches(self):
        sha = "abc1234567890abcdef"
        with (
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_main_branch_sha", return_value=sha),
            patch(
                "src.gh_pr_phase_monitor.monitor.pages_watcher.get_pages_latest_build",
    ...
    def test_pending_when_build_sha_differs(self):
        main_sha = "abc1234567890abcdef"
        build_sha = "def456"
        with (
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_main_branch_sha", return_value=main_sha),
            patch(
                "src.gh_pr_phase_monitor.monitor.pages_watcher.get_pages_latest_build",
    ...
    def test_errored_when_build_status_errored(self):
        sha = "abc1234567890abcdef"
        with (
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_main_branch_sha", return_value=sha),
            patch(
    ...
    def test_unknown_when_main_sha_unavailable(self):
        with patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_main_branch_sha", return_value=None):
            result = check_pages_deployment("owner", "repo")
            assert result["status"] == "unknown"
            assert result["sha"] is None

    def test_pending_when_no_build_found(self):
        sha = "abc1234567890abcdef"
        with (
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_main_branch_sha", return_value=sha),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_pages_latest_build", return_value=None),
        ):
            result = check_pages_deployment("owner", "repo")
            assert result["status"] == "pending"
            assert result["sha"] == sha
...
class TestProcessPagesDeployment:
    ...
    def test_opens_browser_when_deployed_and_enabled(self):
        sha = "abc1234567890abcdef"
        ...
        with (
            patch(
                "src.gh_pr_phase_monitor.monitor.pages_watcher.check_pages_deployment",
                return_value={"status": "deployed", "sha": sha, "build_sha": sha, "build_status": "built"},
            ),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_pages_url", return_value=pages_url),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._can_open_browser", return_value=True),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._should_autoraise_window", return_value=True),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._record_browser_open") as mock_record,
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.webbrowser.open") as mock_open,
        ):
            process_pages_deployment("owner", "test-repo", config)
            mock_open.assert_called_once_with(pages_url, autoraise=True)
            mock_record.assert_called_once()

    def test_dry_run_when_deployed_and_disabled(self, capsys):
        sha = "abc1234567890abcdef"
        pages_url = "https://owner.github.io/test-repo/"
    ...
    def test_browser_not_opened_twice_for_same_sha(self):
        sha = "abc1234567890abcdef"
        pages_url = "https://owner.github.io/test-repo/"
        config = self._make_config(True)
        ...
        with (
            ...
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._can_open_browser", return_value=True),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._should_autoraise_window", return_value=True),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._record_browser_open"),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.webbrowser.open") as mock_open,
        ):
    ...
    def test_opens_actions_when_errored_and_enabled(self):
        sha = "abc1234567890abcdef"
        config = self._make_config(True)
    ...
    def test_dry_run_when_errored_and_disabled(self, capsys):
        sha = "abc1234567890abcdef"
        config = self._make_config(False)
        ...
        with patch(
            ...
            with patch("src.gh_pr_phase_monitor.monitor.pages_watcher.webbrowser.open") as mock_open:
                process_pages_deployment("owner", "test-repo", config)
                mock_open.assert_not_called()
                captured = capsys.readouterr()
                assert "[DRY-RUN]" in captured.out
    ...
    def test_shows_pending_message_each_iteration(self, capsys):
        sha = "abc1234"
        build_sha = "def456"
        config = self._make_config(True)
    ...
    def test_unknown_status_produces_no_output(self, capsys):
        config = self._make_config(True)
    ...
    def test_cooldown_prevents_browser_open(self, capsys):
        sha = "abc1234567890abcdef"
        pages_url = "https://owner.github.io/test-repo/"
        config = self._make_config(True)
        ...
        with (
            ...
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.get_pages_url", return_value=pages_url),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._can_open_browser", return_value=False),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher._get_remaining_cooldown", return_value=45.0),
            patch("src.gh_pr_phase_monitor.monitor.pages_watcher.webbrowser.open") as mock_open,
        ):
            process_pages_deployment("owner", "test-repo", config)
            mock_open.assert_not_called()
...
class TestCheckPagesDeploymentsForRepos:
    ...
    def test_processes_all_repos_in_list(self):
        repos = [
        ...
        with patch("src.gh_pr_phase_monitor.monitor.pages_watcher.process_pages_deployment") as mock_process:
    ...
    def test_handles_exception_from_process(self, capsys):
        ...
        with patch(
            "src.gh_pr_phase_monitor.monitor.pages_watcher.process_pages_deployment",
