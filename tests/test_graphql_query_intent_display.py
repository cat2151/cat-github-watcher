...
def test_intent_is_printed_before_query(capsys):
    response = {"data": {"viewer": {"login": "testuser"}}}
    with patch("subprocess.run", return_value=_make_subprocess_result(response)):
...
def test_no_intent_prints_nothing_extra(capsys):
    response = {"data": {"viewer": {"login": "testuser"}}}
    with patch("subprocess.run", return_value=_make_subprocess_result(response)):
...
def test_cost_is_printed_when_rate_limit_in_response(capsys):
    response = {
        "data": {
...
def test_cost_not_printed_when_no_rate_limit_in_response(capsys):
    response = {"data": {"viewer": {"login": "testuser"}}}
    with patch("subprocess.run", return_value=_make_subprocess_result(response)):
        execute_graphql_query("query { viewer { login } }")
...
def test_intent_and_cost_both_printed_together(capsys):
    response = {
        "data": {
