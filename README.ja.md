...
- **ローカルリポジトリpull検知**: デフォルトで親ディレクトリにある自分のリポジトリのpull可能状態を表示（Dry-run）。`enable_execution_git_pull = true`を設定すると自動pullする（[cat-repo-auditor](https://github.com/cat2151/cat-repo-auditor)参考実装）
...
3. `config.toml` を編集して、監視間隔や実行モード、ntfy.sh通知、Copilot自動割り当て、自動マージを設定（オプション）：
   ...
   enable_execution_git_pull = false
