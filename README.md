...
- **Self-update**: Always performs an update check on startup and automatically pulls updates and restarts if available. Furthermore, setting `enable_auto_update = true` continues update checks every minute even during the monitoring loop (disabled by default).
