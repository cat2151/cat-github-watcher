...
- **省電力モード**: 状態変化がない場合に監視間隔を自動延長する機能（`no_change_timeout`と`reduced_frequency_interval`で設定可能）。デフォルトでは無効（ETagにより304 Not Modifiedレスポンスのチェックはクエリ消費がないため1分ごとに実施）
...
6. **繰り返し**: 設定された間隔で監視を継続
   - ETagにより304 Not Modifiedレスポンスはクエリ消費なしのため、デフォルトでは1分ごとに監視を継続
   - `no_change_timeout`を設定した場合、状態変化がない状態がその時間続いた場合に自動的に省電力モード（`reduced_frequency_interval`）に切り替わりAPI使用量を削減
   - 変化が検知されると通常の監視間隔に戻る
