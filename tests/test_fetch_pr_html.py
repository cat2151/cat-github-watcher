...
from src.gh_pr_phase_monitor.phase.html.pr_html_saver import fetch_pr_html, parse_pr_url, save_html_to_logs, save_pr_html
...
class TestFetchPrHtml:
    def _make_run(self, mocker, returncode=0, stdout=""):
        mock = mocker.MagicMock()
        mock.returncode = returncode
        mock.stdout = stdout
        return mock

    def test_returns_html_on_success(self, mocker):
        html_body = "<html><body>PR content</body></html>"
        curl_mock = self._make_run(mocker, returncode=0, stdout=f"{html_body}\n200")

        mocker.patch("src.gh_pr_phase_monitor.phase.html.pr_html_fetcher.subprocess.run", return_value=curl_mock)
        result = fetch_pr_html("https://github.com/owner/repo/pull/1")

        assert result == html_body

    def test_returns_none_on_curl_failure(self, mocker):
        curl_mock = self._make_run(mocker, returncode=1, stdout="")
    ...
    def test_returns_none_on_non_2xx_status(self, mocker):
    ...
    def test_returns_none_on_subprocess_exception(self, mocker):
        mocker.patch("src.gh_pr_phase_monitor.phase.html.pr_html_fetcher.subprocess.run", side_effect=OSError("no curl"))
        result = fetch_pr_html("https://github.com/owner/repo/pull/1")
...
class TestSavePrHtml:
    def test_saves_file_with_correct_name(self, mocker, tmp_path):
        html_content = "<html><body>PR</body></html>"
        curl_mock = mocker.MagicMock(returncode=0, stdout=f"{html_content}\n200")
    ...
    def test_returns_none_when_fetch_fails(self, mocker, tmp_path):
        curl_mock = mocker.MagicMock(returncode=1, stdout="")
...
class TestSaveHtmlToLogs:
    ...
    def test_uses_pre_computed_analysis_without_reanalysis(self, mocker, tmp_path):
        html = "<html><body>PR</body></html>"
        precomputed = {"pr_url": "https://github.com/o/r/pull/1", "is_draft": True, "llm_statuses": [], "status": "PHASE1A_DRAFT_LLM_WORKING"}
        mock_analyze = mocker.patch("src.gh_pr_phase_monitor.phase.html.pr_html_saver.analyze_pr_html")
...
class TestMainFetchPrHtmlOption:
    ...
    def test_exits_zero_on_success(self, mocker, tmp_path):
        ...
        try:
            mocker.patch(
                "src.gh_pr_phase_monitor.phase.html.pr_html_saver.save_pr_html",
    ...
    def test_exits_one_on_failure(self, mocker):
        ...
        sys.argv = ["cat-github-watcher.py", "--fetch-pr-html", "https://github.com/o/repo/pull/1"]
        try:
            mocker.patch("src.gh_pr_phase_monitor.phase.html.pr_html_saver.save_pr_html", return_value=None)
            from src.gh_pr_phase_monitor.main import main
