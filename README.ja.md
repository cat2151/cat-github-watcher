...
## 運用メモ（エージェント向け）
- ユーザーから具体的なPR/issue URLが提示された場合は、内部コード検討より先に当該URLの状態を確認し、事実ベースで状況整理する。
- インストールやテスト実行が失敗したときは、1回目の失敗で打ち切り、エラー内容を報告して指示を仰ぐ（同一失敗の無限リトライは禁止）。
- 必要性が不明な追加インストールや重い実行は行わず、事前に必要性を確認する。環境制約で実行できない場合は即座に報告する。
- PRタイトルとPR冒頭は「コメント対応」ではなく、PR全体の達成事項（中核的な改善内容）を簡潔に記述する。
