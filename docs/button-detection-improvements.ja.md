...
### 3. HTML要素ベースの検出（実験的・未統合）

Playwrightを使用してHTML要素を直接検出する関数が実装されていますが、**現在のフォールバックチェーンには統合されていません**。

**現状:**
- 関数 `_click_button_with_html` は実装済みですが、画像認識→OCRのフォールバックフローから呼び出されていません
- 設定フラグ `enable_html_detection` は将来の拡張用に予約されており、現時点では効果がありません

**技術的な課題:**
- ブラウザをリモートデバッグモード（`--remote-debugging-port=9222`）で起動する必要がある
- ブラウザの起動方法がユーザー環境に依存するため、統合が複雑

**将来の統合計画:**
この機能は、OCR検出が失敗した場合の追加フォールバックとして統合される予定です。
