...
- **自己更新**: 起動時に必ず更新有無を確認して表示する。`enable_auto_update = true` のときだけ自動pullと再起動を行う。`enable_auto_update = false`（デフォルト、未設定含む）のときは更新を検知・表示するだけで適用しない。true の場合は監視ループ中も1分ごとに更新チェックを継続する。
