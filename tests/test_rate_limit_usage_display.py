...
def test_display_rate_limit_usage_shows_consumed_and_remaining(capsys):
    before = _make_rate_limit(remaining=4800)
    after = _make_rate_limit(remaining=4758)

    with patch(
        "src.gh_pr_phase_monitor.github.rate_limit_handler._format_rate_limit_reset",
...
def test_display_rate_limit_usage_no_before_omits_consumed(capsys):
...
def test_display_rate_limit_usage_zero_consumed(capsys):
